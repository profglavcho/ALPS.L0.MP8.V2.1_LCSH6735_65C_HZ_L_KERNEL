...
void md_cd_traffic_monitor_func(unsigned long data)
{
    ...
    CCCI_INF_MSG(md->index, TAG, "traffic(net_tx): [%d]%ld %ld, [%d]%ld %ld, [%d]%ld %ld\n", 
	...
	CCCI_INF_MSG(md->index, TAG, "traffic(net_rx): [%d]%ld, [%d]%ld, [%d]%ld\n", 
...
/*
    if( (md_ctrl->logic_ch_pkt_pre_cnt[CCCI_CCMNI1_TX]- md_ctrl->logic_ch_pkt_cnt[CCCI_CCMNI1_TX] > 50) ||
        (md_ctrl->logic_ch_pkt_pre_cnt[CCCI_CCMNI2_TX]- md_ctrl->logic_ch_pkt_cnt[CCCI_CCMNI2_TX] > 50) ||
        (md_ctrl->logic_ch_pkt_pre_cnt[CCCI_CCMNI3_TX]- md_ctrl->logic_ch_pkt_cnt[CCCI_CCMNI3_TX] > 50))
    {    
        CCCI_INF_MSG(md->index, TAG, "Net tx pending too long time, dump cldma\n");
        cldma_dump_all_gpd(md);
        cldma_dump_register(md);
        cldma_dump_packet_history(md);
        CCCI_INF_MSG(md->index, TAG, "Net tx pending too long time, force assert\n");
    ...
    *result = UNDER_BUDGET;
    ...
    while(1) { // not hardware own
        ...
        if(md->md_state == GATED||md->md_state == RESET||md->md_state == INVALID){
            ...
            ccci_dump_log_add(md,IN,(int)queue->index, ccci_h,1);
...
fill_and_move:
            ...
            md->rx_history_ptr[queue->index]--;
            md->rx_history_ptr[queue->index] &= (PACKET_HISTORY_DEPTH-1);
#endif
            *result = PORT_REFUSE;
            break;
        }
...
static enum hrtimer_restart cldma_tx_done_timer(struct hrtimer *timer)
{
    ...
    if(count){
        CCCI_DBG_MSG(md->index, TAG, "cldma_tx_done_timer: %d\n", count);
    }
	if(count) {
...
#ifdef CONFIG_ARCH_MT6735M //tmp disable
	return;
#endif
    // 2. wakelock
...
#if defined (CONFIG_MTK_AEE_FEATURE)
				aee_kernel_dal_show("After AP send EPOF, MD didn't go to sleep in 4 seconds.\n");
#endif
...
retry:
    ...
    if(queue->free_slot > 0) {
        ...
        if(md_ctrl->txq_active & (1<<qno)) {
            ...
            CCCI_ERR_MSG(md->index, TAG, "md_ctrl->txq_active=%d, qno=%d is 0\n", md_ctrl->txq_active,qno);
        }
...
{
    int count = 0;
    count = snprintf(buf, 256, "support: cldma_reset cldma_stop ccif_assert md_type_store\n");
    return count;
}
