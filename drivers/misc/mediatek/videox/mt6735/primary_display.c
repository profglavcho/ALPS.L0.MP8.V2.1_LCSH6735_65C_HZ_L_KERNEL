...
#ifdef DISABLE_SODI_IN_VDO
extern void defeature_soidle_by_display_video_mode(int id);
#endif
...
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	{
		if (atomic_read(&need_wait_te))
		{
			dpmgr_path_build_cmdq(pgc->dpmgr_handle, pgc->cmdq_handle_config, CMDQ_BEFORE_STREAM_SOF);
			atomic_set(&need_wait_te, 0);
			DISPMSG("Vsync period over 20msec.\n");
		}
		_cmdq_set_config_handle_dirty();
...
static atomic_t listener_wakeup = ATOMIC_INIT(0); // for set_listener update
static unsigned long listener_curr = 0, listener_prev = 0;
static unsigned long listener_diff = 0;
...
static int _disp_primary_path_check_trigger(void *data)
{
	...
	{	
		...
		// avoid too fast update to overdrive the hw limit
		if(atomic_read(&listener_wakeup))
		{
			DISPERR("too fast update: critical section!! \n");
			continue;
		}
		atomic_set(&listener_wakeup, 1);

		listener_curr = get_current_time_us();
		listener_diff = listener_curr - listener_prev;
		if (listener_diff < 13*1000)
		{
			DISPERR("too fast update: %ld\n", listener_diff);
			continue;
		}
...
static void _RDMA0_INTERNAL_IRQ_Handler(DISP_MODULE_ENUM module, unsigned int param)
{
	if(!_is_decouple_mode(pgc->session_mode) && param & 0x2)
	{
		...
		atomic_set(&listener_wakeup, 0);
		listener_prev = get_current_time_us();
	}
	if(param & 0x4)
	{
		// RDMA Done
...
//	dpmgr_path_init(pgc->dpmgr_handle, CMDQ_DISABLE);
    ...
    if (primary_display_is_video_mode() == 1)
    {
        defeature_soidle_by_display_video_mode(1);
    }
#endif
	_primary_path_unlock(__func__);
