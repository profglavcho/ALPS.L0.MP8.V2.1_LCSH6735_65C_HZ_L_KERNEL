...
/*MCLK counter*/
static MINT32 mMclk1User = 0;
static MINT32 mMclk2User = 0;
static MINT32 mMclk3User = 0;
/**********************************************
************************************************/
#ifdef  T_STAMP_2_0
...
//      spin_unlock_irqrestore(&(IspInfo.SpinLock), flags);
	...
	LOG_INF("[rc]QQ return value:(%d)",Ret);// for memory corruption check
    //
    ////////////////////---add unlock here
//  spin_unlock_irqrestore(&(IspInfo.SpinLock), flags);
    ////////////////////
            ...
            if(copy_from_user(&IrqInfo, (void*)Param, sizeof(ISP_WAIT_IRQ_STRUCT)) == 0)
            {
                ...
                {   //isp driver related operation in v1 is redirected to v3 flow cuz userNumer and default UserKey are 0
                    ...
                    if((IrqInfo.UserInfo.UserKey>=IRQ_USER_NUM_MAX) || ((IrqInfo.UserInfo.UserKey<=0) && (IrqInfo.UserNumber==0)))
                    {
                        ...
                        userKey=0;    //force to use default user key if the passed key is invalid or driver in v1
                    }
        ...
        case ISP_FLUSH_IRQ_REQUEST:
            if(copy_from_user(&IrqInfo, (void*)Param, sizeof(ISP_WAIT_IRQ_STRUCT)) == 0)
            {
                ...
                {//v1 flow / v1 ISP_IRQ_USER_MAX
                    ...
                    if((IrqInfo.Type>=ISP_IRQ_TYPE_AMOUNT) || (IrqInfo.Type<0))
                    {
                        LOG_ERR("invalid type(%d), max(%d)",IrqInfo.Type,ISP_IRQ_TYPE_AMOUNT);
                        Ret = -EFAULT;
                        break;
                    }
                    //check UserNumber
                    ...
                    {
                        LOG_ERR("invalid userNumber(%d)",IrqInfo.UserNumber);
                        Ret = -EFAULT;
                        break;
                    }
                    if( 0 == (IrqInfo.Type|IrqFlush[IrqInfo.UserNumber]))
                    {
                        LOG_ERR("UserNumber(%d), invalid type(%d)",IrqInfo.UserNumber,IrqInfo.Type);
                        Ret = -EFAULT;
                        break;
                    }
                    LOG_INF("UserNumber(%d), type(%d)",IrqInfo.UserNumber,IrqInfo.Type);
                    Ret = ISP_FLUSH_IRQ(IrqInfo);
                }
                else
                {//v3 flow /v3 IRQ_USER_NUM_MAX
                    if((IrqInfo.UserInfo.UserKey>=IRQ_USER_NUM_MAX) || (IrqInfo.UserInfo.UserKey<1))
                    {
                        LOG_ERR("invalid userKey(%d), max(%d)",IrqInfo.UserInfo.UserKey,IRQ_USER_NUM_MAX);
                        Ret = -EFAULT;
                    ...
                    {
                        LOG_ERR("User_%s(%d), invalid type(%d)",IrqUserKey_UserInfo[IrqInfo.UserInfo.UserKey].userName,IrqInfo.UserInfo.UserKey,IrqInfo.UserInfo.Type);
                        Ret = -EFAULT;
                        break;
                    }
                    LOG_INF("User_%s(%d), type(%d)",IrqUserKey_UserInfo[IrqInfo.UserInfo.UserKey].userName,IrqInfo.UserInfo.UserKey,IrqInfo.UserInfo.Type);
    ...
    struct file*    pFile)
{
    ...
    IrqFlush[ISP_IRQ_USER_3A] = (ISP_IRQ_P1_STATUS_VS1_INT_ST | ISP_IRQ_P1_STATUS_D_VS1_INT_ST |\
                                 ISP_IRQ_P1_STATUS_AF_DON_ST | ISP_IRQ_P1_STATUS_D_AF_DON_ST);
    IrqFlush[ISP_IRQ_USER_EIS] = (ISP_IRQ_P1_STATUS_VS1_INT_ST | ISP_IRQ_P1_STATUS_D_VS1_INT_ST | \
                                  ISP_IRQ_P1_STATUS_PASS1_DON_ST | ISP_IRQ_P1_STATUS_D_PASS1_DON_ST);
    IrqFlush[ISP_IRQ_USER_VHDR] = (ISP_IRQ_P1_STATUS_VS1_INT_ST | ISP_IRQ_P1_STATUS_D_VS1_INT_ST | \
                                   ISP_IRQ_P1_STATUS_PASS1_DON_ST | ISP_IRQ_P1_STATUS_D_PASS1_DON_ST);
    //
    for(i=0;i<_MAX_SUPPORT_P2_FRAME_NUM_;i++)
    {
