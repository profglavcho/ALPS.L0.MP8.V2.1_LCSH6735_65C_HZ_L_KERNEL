...
#ifdef CMDQ_PRINT_ERRORDUMP0
bool gCmdqPrintDumpError0_Flag = false;
char gCmdqPrintDumpError0_Buffer[CMDQ_MAX_PRINTERRORDUMP0_COUNT] = {""};
uint32_t gCmdqPrintDumpError0_Count = 0;
#endif
...
#ifdef CMDQ_PRINT_ERRORDUMP0
	if (gCmdqPrintDumpError0_Count > 0){
		seq_printf(m, "\n\n%s", gCmdqPrintDumpError0_Buffer);
		seq_printf(m, "Error0 Dump Count = %d\n", gCmdqPrintDumpError0_Count);
	}
#endif
					      ...
					      CMDQ_SCENARIO_ENUM scenario, bool forceLog, const bool isSecure)
{
	...
	do {
		...
		if (CMDQ_INVALID_THREAD != thread) {
			...
			if ( pThread[thread].pCurTask[ pThread[thread].nextCookie % CMDQ_MAX_TASK_IN_THREAD ] != NULL ) {
				if (forceLog) {
...
static void cmdq_core_attach_error_task(const TaskStruct *pTask, int32_t thread)
{
	...
	if (0 == gCmdqContext.errNum){
		gCmdqPrintDumpError0_Flag = true;
	}
	else if (true == gCmdqPrintDumpError0_Flag) {
		gCmdqPrintDumpError0_Flag = false;
	}
#endif
...
int32_t cmdq_core_save_dump_error0(const char *string, ...)
{
#ifdef CMDQ_PRINT_ERRORDUMP0
	if (true == gCmdqPrintDumpError0_Flag){
		va_list argptr;
		va_start(argptr, string);
    	int logLen = vsnprintf(NULL, 0, string, argptr)+1;
		if (logLen>1) {
			if ( (gCmdqPrintDumpError0_Count+logLen)<CMDQ_MAX_PRINTERRORDUMP0_COUNT ) {
				char *pBuffer = gCmdqPrintDumpError0_Buffer + gCmdqPrintDumpError0_Count;
				gCmdqPrintDumpError0_Count += vsnprintf(pBuffer, logLen, string, argptr);
			}
			else {
				gCmdqPrintDumpError0_Flag = false;
