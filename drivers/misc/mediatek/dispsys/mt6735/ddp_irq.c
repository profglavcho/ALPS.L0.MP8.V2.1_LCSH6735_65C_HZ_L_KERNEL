...
unsigned long te_curr = 0, te_prev = 0;
atomic_t need_wait_te = ATOMIC_INIT(0);
...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
    	...
    	if(irq==dispsys_irq[DISP_REG_DSI0])
	{
            ...
            if (reg_val&(1<<2))
            {
                ...
                if (te_curr == 0 || te_prev == 0)
                {
                    te_curr = get_current_time_us();
                    te_prev = get_current_time_us();
                }
                else
                {
                    te_prev = te_curr;
                    te_curr = get_current_time_us();
                    if ((te_curr - te_prev) > 20000)
                        atomic_set(&need_wait_te, 1);
                }
            }
