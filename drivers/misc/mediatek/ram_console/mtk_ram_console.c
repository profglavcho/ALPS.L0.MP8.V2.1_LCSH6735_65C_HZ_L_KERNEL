...
static int ram_console2_show(struct seq_file *m, void *v)
{
	...
	if (bufp->off_console < bufp->sz_buffer)
...
static int emmc_read_last_kmsg(void *data)
{
	...
	size = ram_console_buffer->sz_buffer;
	if (size == 0) {
		pr_err("ram_console: ram_console buffer_size = 0\n");
		return 1;
	}
	...
	filp = expdb_open();
	if (!IS_ERR(filp)) {
		ret = kernel_read(filp, EMMC_ADDR, ram_console2_log, size);
		fput(filp);
...
static int __init ram_console_init(struct ram_console_buffer *buffer, size_t buffer_size)
{
	...
	buffer->sz_console = buffer_size - buffer->off_console;
