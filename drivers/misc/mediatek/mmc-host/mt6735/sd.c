...
static void msdc_dump_clock_sts(struct msdc_host *host)
{
	...
	if(apmixed_reg_base1 && topckgen_reg_base ){ 
		...
		pr_err(" PERI_PDN_STA0   [0x%p][bit13=msdc0, bit14=msdc1,0:on,1:off]=0x%x\n",
			(topckgen_reg_base + MSDC_PERI_PDN_STA0_OFFSET), sdr_read32(topckgen_reg_base + MSDC_PERI_PDN_STA0_OFFSET));
	}else
		pr_err(" apmixed_reg_base1 = %p, topckgen_reg_base = %p\n", apmixed_reg_base1,topckgen_reg_base);
#endif
}
...
static void msdc_dump_ldo_sts(struct msdc_host *host)
{
    ...
    switch(host->id){
        case 0:
            ...
            SIMPLE_INIT_MSG(" VEMC_EN[0x0A24]=0x%x, should:bit1=1, VEMC_VOL[0x0A64]=0x%x,should:[bit[5:4]=2b'01]\n", ldo_en, ldo_vol);
            break;
        case 1:
    ...
    switch(host->id){
        case 1:
            ...
            if (host->hw->flags & MSDC_SD_NEED_POWER) {
                msdc_ldo_power(1, MT6328_POWER_LDO_VMC, VOL_3300, &g_msdc1_io);
...
stop:
	...
	if (mrq->cmd->error == (unsigned int)-ETIMEDOUT) {
		if ((cmd->opcode == MMC_SLEEP_AWAKE) && (host->hw->host_function == MSDC_EMMC)) {
			emmc_sleep_failed = 1;
			mrq->cmd->error = 0x0;
		} else {
